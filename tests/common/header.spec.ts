...
test.describe("Header - V Learning", () => {
  ...
  test("TC-005 Kiểm tra hiển thị của nút [Đăng nhập]", async ({ page }) => {
    const header = new Header(page);
    await header.verifyVisible(header.loginButton);
  });

  test("TC-006 Kiểm tra liên kết của nút [Đăng nhập]", async ({ page }) => {
    const header = new Header(page);
    await header.loginButton.click();
    await expect(page).toHaveURL(/.*\/login/);
  });

  test("TC-007 Kiểm tra hiển thị của ô [Tìm kiếm]", async ({ page }) => {
    const header = new Header(page);
    await header.verifyVisible(header.searchBox);
  });

  test("TC-009 Kiểm tra giao diện (UI) của ô [Tìm kiếm]", async ({ page }) => {
    const header = new Header(page);
    await header.verifySearchBoxUI();
  });
